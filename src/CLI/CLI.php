...
class CLI
{
	...
	public function __construct(array $argv, Text $text)
	{
		...
		$this->setChannel(new StdoutChannel($this->text));
		$this->setChannel(new StderrChannel($this->text));

		// Create a custom debug channel and disable it by default
	...
	public function runProcess(string $command, ChannelInterface $stdout, ChannelInterface $stderr): string
	{
		...
		if($stdout instanceof HistoryChannelInterface){
			self::$stdout = trim(implode("\n", $stdout->getRendered()));
		}
		
		if($stderr instanceof HistoryChannelInterface){
	...
	public function exec(string $command, ?ChannelInterface $stdout=null, ?ChannelInterface $stderr=null): string
	{
		...
		$this->getChannel('debug')->write("{red}[EXEC(>>ctr:$ctr)]:{end} $command\n");

		$output = $this->runProcess($command, $stdout, $stderr);

		$code = $this->getExitCode();

		$this->getChannel('debug')->write("{red}[EXEC(<<ctr:$ctr)]:{end} {blu}Return Code:{end} $code {blu}Error Output:{end} '".self::$stderr."'");
	...
	{
		return $this->getChannel('stdout')->write($string);
	}

	public function stderr(?string $string=''): string
	{
		return $this->getChannel('stderr')->write($this->text->write($string));
	}

	public function debug(string $type, ?string $string='', ?array $params=[])
	{
		return $this->getChannel('debug')->write("{red}[".strtoupper($type)."]{end}: " . $string, $params);
	}
